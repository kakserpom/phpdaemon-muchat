...
class Chat extends AppInstance
{
    ...
    public function kickUsers($users,$tags = '',$reason = '')
    {
        ...
        $tags = trim($tags);
        $this->broadcastEvent(array('type' => 'kickUsers',
        'users' => explode(',',$users),
    ...
    public function forceChangeNick($name,$newname)
    {
        ...
        $this->broadcastEvent(array('type' => 'forceChangeNick',
        'username' => $name,
        'changeto' => $newname,
        'tags' => '%all',
        ));
    ...
    public function broadcastEvent($doc)
    {
        ...
        $this->db->{
        $this->config->dbname->value.'.chatevents'}
        ->insert($doc);
    }
...
class ChatTag
{
    ...
    public function touch()
    {
        if (!$this->cursor) {
            ...
            ->find(function($cursor) use($tag)
            {
                ...
                foreach($cursor->items as $k => &$item)
                {
                    if ($item['type'] === 'kickUsers') {
                        foreach($tag->sessions as $id => $v)
                        {
                            ...
                            if (($sess->username !== NULL) && ($tag->appInstance->compareMask($sess->username,$item['users']))) {
                                ...
                                $tag->appInstance->broadcastEvent(array('type' => 'msg',
                                'mtype' => 'system',
                    ...
                    } else if ($item['type'] === 'forceChangeNick') {
                        foreach($tag->sessions as $id => $v)
                        {
                            $sess = $tag->appInstance->sessions[$id];
        ...
        } else if (!$this->cursor->session->busy) {
            try {
                $this->cursor->getMore();
            }
...
class ChatSession
{
    ...
    public function onAddedTags($tags,$silence = FALSE)
    {
        ...
        if ($this->username !== NULL) {
            $this->broadcastEvent(array('type' => 'joinsUser',
            'sid' => (string) $this->sid,
            ...
            if (!$silence) {
                $this->broadcastEvent(array('type' => 'msg',
                'mtype' => 'system',
    ...
    public function onRemovedTags($tags,$silence = FALSE)
    {
        ...
        if ($this->username !== NULL) {
            $this->broadcastEvent(array('type' => 'partsUser',
            'sid' => (string) $this->sid,
            'username' => $this->username,
            'tags' => $tags,
            ));
    ...
    public function updateSession($a)
    {
        ...
        $this->appInstance->db->{
        $this->appInstance->config->dbname->value.'.chatsessions'}
        ->upsert(array('id' => $this->sid),
        array('$set' => $a)
        );
        if (isset($a['statusmsg'])) {
            $this->sendMessage(array('mtype' => 'status',
            'tags' => $this->tags,
            'from' => $this->username,
            'mtype' => 'status',
            'text' => $a['statusmsg'],
    ...
    public function setUsername($name,$silence = FALSE)
    {
        ...
        $this->appInstance->db->{
        $this->appInstance->config->dbname->value.'.chatsessions'}
        ->findOne(function($item) use($clientId, $appInstance, $name, $silence)
        {
            ...
            $session->updateSession(array('atime' => microtime(TRUE),
            'username' => $name,
            ));
            $session->send(array('type' => 'cstatus',
            'username' => $name,
            ));
            if ($session->username !== NULL) {
                $session->broadcastEvent(array('type' => 'msg',
                'mtype' => 'astatus',
                'from' => $session->username,
                'text' => 'is now known as '.$name,
                'color' => 'green',
                ));
                $session->broadcastEvent(array('type' => 'changedUsername',
                'sid' => (string) $session->sid,
                'old' => $session->username,
                'new' => $name,
                ));
            } else {
                $session->broadcastEvent(array('type' => 'joinsUser',
                'sid' => (string) $session->sid,
                'username' => $name,
                'tags' => $session->tags,
        ...
        ,array('where' => array('username' => $name,
    ...
    public function onFrame($data,$type)
    {
        ...
        if (($cmd === 'setUsername') && isset($packet['username'])) {
            ...
            $this->appInstance->db->{
            $this->appInstance->config->dbname->value.'.chatevents'}
            ->find(function($cursor) use($session)
            {
                $tag->cursor = $cursor;
            ...
            ,array('sort' => array('ctime' => -1),
            'where' => array('tags' => array('$in' => $packet['tags']),
            'atime' => array('$gt' => microtime(TRUE)-20),
            ...
            if ($c === '/') {
                ...
                if ($m === 'me') {
                    ...
                    } else if (!$this->appInstance->validateUsername($newname)) {
