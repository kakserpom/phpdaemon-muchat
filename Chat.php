...
*/
	...
	class ChatSession extends WebSocketRoute{
		...
		public function onFinish() {
			...
			unset($this->appInstance->sessions[$this->client->id]);
		}

		public function onAddedTags($tags,$silence = FALSE) {
			foreach ($tags as $tag)  {
				++$this->appInstance->getTag($tag)->counter;
				$this->appInstance->getTag($tag)->sessions[$this->client->id] = TRUE;
			}
			...
			if ($this->username !== NULL)  {
				...
				if (!$silence)   {
					$this->broadcastEvent(array(     'type' => 'msg',     'mtype' => 'system',     'text' => '* Joins: '.$this->username,     'color' => 'green',     'tags' => $tags,    ));
				}
			}
		}
		...
		public function onRemovedTags($tags,$silence = FALSE) {
			foreach ($tags as $tag)  {
				--$this->appInstance->getTag($tag)->counter;
				unset($this->appInstance->tags[$tag]->sessions[$this->client->id]);
			}			
			if ($this->username !== NULL)  {
				$this->broadcastEvent(array(   'type' => 'partsUser',    'sid' => (string) $this->sid,    'username' => $this->username,    'tags' => $tags,   ));
				if (!$silence)   {
					$this->broadcastEvent(array(     'type' => 'msg',     'mtype' => 'system',     'text' => '* Parts: '.$this->username,     'color' => 'green',     'tags' => $tags,    ));
				}
			}
		}
		...
		public function setUsername($name,$silence = FALSE) {
			...
			if ($name === '')  {
				if (!$silence) {
					$this->sysMsg('* /nick <name>: insufficient parameters');
				}
				return 4;
			}
			...
			$this->appInstance->db->{$this->appInstance->config->dbname->value.'.chatsessions'}->findOne(
				function($item) use ($clientId, $appInstance, $name, $silence)  {
				...
				$session->updateSession(array(     'atime' => microtime(TRUE),     'username' => $name,   ));
				$session->send(array(     'type' => 'cstatus',     'username' => $name,   ));
				
				if ($session->username !== NULL)   {
					$session->broadcastEvent(array(     'type' => 'msg',     'mtype' => 'astatus',     'from' => $session->username,     'text' => 'is now known as '.$name,     'color' => 'green',    ));
					$session->broadcastEvent(array(     'type' => 'changedUsername',     'sid' => (string) $session->sid,     'old' => $session->username,     'new' => $name,    ));
				} else   {
					$session->broadcastEvent(array(     'type' => 'joinsUser',     'sid' => (string) $session->sid,     'username' => $name,     'tags' => $session->tags,     'statusmsg' => $session->statusmsg,    ));
					$session->broadcastEvent(array(     'type' => 'msg',     'mtype' => 'system',     'text' => '* Joins: '.$name,     'color' => 'green',    ));
				}

				$session->username = $name;
			}, array('where' => array(    'username' => $name,    'atime' => array('$gt' => microtime(TRUE)-20),)));
			return 1;
		}
		...
		public function onFrame($data,$type) {
			...
			elseif ($cmd == 'getUserlist')  {
				$session = $this;
				$this->appInstance->db->{
					$this->appInstance->config->dbname->value.'.chatsessions'}

				->find(
				function($cursor) use ($session)   {
					$cursor->items = array_reverse($cursor->items);
					foreach ($cursor->items as $k => &$item)    {
						unset($item['_id']);
				...
				,array(    'sort' => array('ctime' => -1),    'where' => array(       'tags' => array('$in' => $packet['tags']),       'atime' => array('$gt' => microtime(TRUE)-20),       'username' => array('$exists' => TRUE),    ),    'limit' => -200,   ));
			}
			...
			elseif ($cmd === 'sendMessage')  {
				...
				if ($c === '/')   {
					...
					if ($m === 'me')    {
						...
						if ($text === '') {
							$this->sysMsg('* /me <message>: insufficient parameters');
						} else     {
					...
					elseif ($m === 'join')    {
						...
						if ($tags !== '') {
							...
							$this->sysMsg('* /join <tag1>{,<tagN>}: insufficient parameters');
						}
					...
					elseif ($m === 'part')    {
						...
						if ($tags !== '') {
							...
							$this->sysMsg('* /part <tag1>{,<tagN>}: insufficient parameters');
						}
					...
					elseif ($m === 'kick')    {
						...
						if ($users === '') {
							$this->sysMsg('* /kick <name> [<tags>] [<reason>]: insufficient parameters');
						} else     {
					...
					elseif ($m === 'fchname')    {
						...
						if (($name === '') || ($newname === '')) {
							$this->sysMsg('* /fchname <name> <newname>: insufficient parameters');
						}

						elseif (!$this->appInstance->validateUsername($newname)) {
							$this->sysMsg('* /fchname: newname>');
						}
					...
					if (preg_match_all('~(?<=^|\s)@([A-Za-z\-_!0-9\.\wА-Яа-яеЁ]+)~u', $text, $m)) {
						$doc['to'] = $m[1];
					}
